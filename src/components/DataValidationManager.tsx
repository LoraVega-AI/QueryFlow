...
export function DataValidationManager({ schema: propSchema, records: propRecords, onSchemaChange }: DataValidationManagerProps) {
  ...
  const renderAnomalies = () => {
    // Get anomalies from both the validation report and the schema
    const reportAnomalies = currentReport?.anomalies || [];
    const schemaAnomalies = schema?.anomalies || [];
    const allAnomalies = [...reportAnomalies, ...schemaAnomalies];
    
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-medium text-gray-900">Project Anomalies</h3>
          <div className="flex items-center space-x-4 text-sm text-gray-600">
            <span>Total: {allAnomalies.length}</span>
            {allAnomalies.length > 0 && (
              <>
                <span>Critical: {allAnomalies.filter(a => a.severity === 'critical').length}</span>
                <span>High: {allAnomalies.filter(a => a.severity === 'high').length}</span>
                <span>Medium: {allAnomalies.filter(a => a.severity === 'medium').length}</span>
                <span>Low: {allAnomalies.filter(a => a.severity === 'low').length}</span>
              </>
            )}
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div key={anomaly.id} className="bg-white rounded-lg p-6 border border-gray-200">
                ...
                <div className="flex items-start">
                  ...
                  <div>
                    <h4 className="text-sm font-medium text-gray-900">
                      {anomaly.title || `${anomaly.type.charAt(0).toUpperCase() + anomaly.type.slice(1)} Anomaly`}
                    </h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                {anomaly.affectedFile && (
                  <div>
                    <span className="font-medium text-gray-700">Affected File:</span>
                    <span className="ml-2 text-gray-900 font-mono text-xs">{anomaly.affectedFile}</span>
                  </div>
                )}
                {anomaly.lineNumber && (
                  <div>
                    <span className="font-medium text-gray-700">Line Number:</span>
                    <span className="ml-2 text-gray-900">{anomaly.lineNumber}</span>
                  </div>
                )}
                {anomaly.confidence && (
                  <div>
                ...
                <div>
                  ...
                  <span className={`ml-2 px-2 py-1 text-xs rounded ${
                    anomaly.type === 'security' ? 'bg-red-100 text-red-800' :
                    anomaly.type === 'performance' ? 'bg-yellow-100 text-yellow-800' :
                    anomaly.type === 'quality' ? 'bg-blue-100 text-blue-800' :
                    anomaly.type === 'architecture' ? 'bg-purple-100 text-purple-800' :
                    'bg-gray-100 text-gray-800'
                  }`}>
                    {anomaly.type}
                  </span>
                </div>
                {anomaly.affectedRecords && (
                  <div>
              ...
              {(anomaly.suggestedAction || anomaly.recommendation) && (
                <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                  <p className="text-sm text-blue-800">
                    <strong>Suggested Action:</strong> {anomaly.suggestedAction || anomaly.recommendation}
                  </p>
                </div>
              )}
            </div>
  ...
  const renderDatabaseMetadata = () => {
    ...
    const databaseInfo = schema.databaseInfo || { type: 'SQLite', version: 'Unknown', encoding: 'UTF-8' };
    const dbInfo = databaseInfo as any; // Type assertion for extended properties
          ...
          <div className="bg-white rounded-lg p-6 border border-gray-200">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-sm text-gray-900">{dbInfo.pageSize || 'Unknown'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-500">Journal Mode:</span>
                <span className="text-sm text-gray-900">{dbInfo.journalMode || 'Unknown'}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-sm text-gray-900">{dbInfo.synchronous || 'Unknown'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-500">Cache Size:</span>
                <span className="text-sm text-gray-900">{dbInfo.cacheSize || 'Unknown'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-500">Auto Vacuum:</span>
                <span className="text-sm text-gray-900">{dbInfo.autoVacuum || 'Unknown'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-500">Created:</span>
                <span className="text-sm text-gray-900">
                  {dbInfo.createdAt ? new Date(dbInfo.createdAt).toLocaleDateString() : 'Unknown'}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-500">Last Modified:</span>
                <span className="text-sm text-gray-900">
                  {dbInfo.updatedAt ? new Date(dbInfo.updatedAt).toLocaleDateString() : 'Unknown'}
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span className="text-sm text-gray-900">
                  {dbInfo.size ? 
                    (dbInfo.size > 1024 * 1024 ? 
                      `${(dbInfo.size / (1024 * 1024)).toFixed(2)} MB` : 
                      `${(dbInfo.size / 1024).toFixed(2)} KB`
                    ) : 
        ...
        {dbInfo.statistics && (
          <div className="bg-white rounded-lg p-6 border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="text-2xl font-bold text-blue-600">{dbInfo.statistics.pageCount || 0}</div>
                <div className="text-sm text-gray-500">Total Pages</div>
              </div>
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="text-2xl font-bold text-green-600">{dbInfo.statistics.freelistCount || 0}</div>
      ...
      table.columns.flatMap(column => {
        ...
        if (column.primaryKey) {
          constraints.push({
            ...
            deferrable: (column as any).deferrable,
            initiallyDeferred: (column as any).initiallyDeferred,
            validated: (column as any).validated,
        ...
        <div className="bg-white rounded-lg border border-gray-200">
          ...
          <div className="divide-y divide-gray-200">
            {models.map((model, index) => (
              <div key={`${model.id}-${index}`} className="p-6 hover:bg-gray-50">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          {model.framework} • {(model as any).filePath || model.filename}
                        </p>
                      </div>
                    </div>
                    
                    {(model as any).description && (
                      <p className="text-sm text-gray-600 mb-3">{(model as any).description}</p>
                    )}
                        ...
                        <div className="space-y-1">
                          {model.relationships?.slice(0, 3).map((rel, relIndex) => (
                            <div key={relIndex} className="flex items-center text-xs">
                              <span className="text-gray-600">{(rel as any).name || (rel as any).target || (rel as any).relatedTable}</span>
                    ...
                    <div className="flex items-center space-x-4 text-xs text-gray-500">
                      <span className="flex items-center">
                        ...
                        {(model as any).filePath || model.filename}
                      </span>
                      {model.tableName && (
                        <span className="flex items-center">
  ...
  const [enhancedProfiles, setEnhancedProfiles] = useState<any[]>([]);
  const [isGeneratingProfiles, setIsGeneratingProfiles] = useState(false);
  const isGeneratingRef = useRef(false);
  
  // Memoize records to prevent unnecessary re-renders
  const memoizedRecords = useMemo(() => records, [records.length, records.map(r => r.id).join(',')]);
  
  // Generate enhanced profiles when data changes
  useEffect(() => {
    const generateEnhancedProfiles = async () => {
      ...
      if (!schema?.tables || memoizedRecords.length === 0) {
        setEnhancedProfiles([]);
        return;
      }
      ...
      try {
        ...
        const profiles = await Promise.all(
          schema.tables.map(async (table) => {
            const tableRecords = memoizedRecords.filter(record => record.tableId === table.id);
            if (tableRecords.length === 0) return null;
            
            console.log(`🔍 Generating enhanced profile for ${table.name}...`);
            return await EnhancedProfileService.generateTableProfile(table, tableRecords, schema);
          })
        );
        
        setEnhancedProfiles(profiles.filter(profile => profile !== null));
      } catch (error) {
        console.error('Error generating enhanced profiles:', error);
        // Fallback to simplified profiles
        setEnhancedProfiles(generateFallbackProfiles());
      } finally {
    ...
    generateEnhancedProfiles();
    
    // Cleanup function
    return () => {
      isGeneratingRef.current = false;
    };
  }, [schema?.id, memoizedRecords.length]); // Use more stable dependencies
  
  // Fallback profile generation for when enhanced service fails
  const generateFallbackProfiles = () => {
    return schema?.tables.map(table => {
      const tableRecords = memoizedRecords.filter(record => record.tableId === table.id);
      const totalRecords = tableRecords.length;
      
      // Basic validation - check for non-null required fields
      const validRecords = tableRecords.filter(record => {
        return table.columns.every(col => {
          const value = record.data?.[col.name];
          return col.nullable || (value !== null && value !== undefined && value !== '');
        });
      });
      
      // Find potential duplicates by checking if all field values are identical
      const duplicateRecords = tableRecords.filter((record, index) => {
        return tableRecords.slice(index + 1).some(otherRecord => 
          JSON.stringify(record.data) === JSON.stringify(otherRecord.data)
        );
      });
      ...
      return {
        ...
        invalidRecords: totalRecords - validRecords.length,
        duplicateRecords: duplicateRecords.length,
        dataCompleteness: totalRecords > 0 ? ((validRecords.length / totalRecords) * 100) : 0,
        dataConsistency: 85, // Placeholder
        referentialIntegrity: 100, // Placeholder
        overallQualityScore: totalRecords > 0 ? ((validRecords.length / totalRecords) * 100) : 0,
        columnProfiles: table.columns.map(col => {
          const columnValues = tableRecords.map(r => r.data?.[col.name]);
          ...
          return {
            columnId: col.id,
            columnName: col.name,
            qualityScore: nonNullValues.length > 0 ? ((uniqueValues.size / nonNullValues.length) * 100) : 0,
            completeness: columnValues.length > 0 ? ((nonNullValues.length / columnValues.length) * 100) : 0,
            uniqueness: nonNullValues.length > 0 ? ((uniqueValues.size / nonNullValues.length) * 100) : 0,
            dataType: col.type,
  ...
  const renderProfiles = () => {
    const tableProfiles = enhancedProfiles.length > 0 ? enhancedProfiles : generateFallbackProfiles();

    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-medium text-gray-900">Enhanced Table Profiles</h3>
          <div className="flex items-center space-x-4">
            ...
            <div className="text-sm text-gray-500">
              {enhancedProfiles.length > 0 ? 'Enhanced analysis' : 
               projectRecords.length > 0 ? 'Basic analysis' : 'Database queries'}
            </div>
          </div>
        </div>
        ...
        {tableProfiles.length > 0 ? (
          <div className="space-y-6">
            {tableProfiles.map((profile) => (
              <div key={profile.tableId} className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                {/* Profile Header */}
                <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="text-xl font-semibold text-gray-900">{profile.tableName}</h4>
                      <p className="text-sm text-gray-600 mt-1">
                        {profile.totalRecords.toLocaleString()} records • Profiled {profile.profiledAt ? new Date(profile.profiledAt).toLocaleString() : 'now'}
                      </p>
                    </div>
                    <div className="flex items-center space-x-4">
                      {renderQualityScore(profile.overallQualityScore || profile.qualityScore)}
                      <div className="text-right">
                        <div className="text-sm text-gray-500">Overall Quality</div>
                        <div className="text-lg font-bold text-gray-900">
                          {Math.round(profile.overallQualityScore || profile.qualityScore)}%
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="p-6">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    <div className="text-center p-4 bg-blue-50 rounded-lg">
                      <div className="text-2xl font-bold text-blue-600">{profile.totalRecords.toLocaleString()}</div>
                      <div className="text-sm text-blue-800">Total Records</div>
                    </div>
                    <div className="text-center p-4 bg-green-50 rounded-lg">
                      <div className="text-2xl font-bold text-green-600">{profile.validRecords.toLocaleString()}</div>
                      <div className="text-sm text-green-800">Valid Records</div>
                      <div className="text-xs text-green-600 mt-1">
                        {profile.totalRecords > 0 ? Math.round((profile.validRecords / profile.totalRecords) * 100) : 0}%
                      </div>
                    </div>
                    <div className="text-center p-4 bg-red-50 rounded-lg">
                      <div className="text-2xl font-bold text-red-600">{profile.invalidRecords.toLocaleString()}</div>
                      <div className="text-sm text-red-800">Invalid Records</div>
                      <div className="text-xs text-red-600 mt-1">
                        {profile.totalRecords > 0 ? Math.round((profile.invalidRecords / profile.totalRecords) * 100) : 0}%
                      </div>
                    </div>
                    <div className="text-center p-4 bg-yellow-50 rounded-lg">
                      <div className="text-2xl font-bold text-yellow-600">{profile.duplicateRecords.toLocaleString()}</div>
                      <div className="text-sm text-yellow-800">Duplicates</div>
                      <div className="text-xs text-yellow-600 mt-1">
                        {profile.totalRecords > 0 ? Math.round((profile.duplicateRecords / profile.totalRecords) * 100) : 0}%
                      </div>
                    </div>
                  </div>
                  
                  {/* Enhanced Quality Metrics */}
                  {(profile.dataCompleteness !== undefined || profile.dataConsistency !== undefined) && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                      <div className="bg-gray-50 rounded-lg p-4">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-sm font-medium text-gray-700">Data Completeness</span>
                          <span className="text-sm font-bold text-gray-900">
                            {Math.round(profile.dataCompleteness || 0)}%
                          </span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div 
                            className="bg-blue-600 h-2 rounded-full" 
                            style={{ width: `${profile.dataCompleteness || 0}%` }}
                          ></div>
                        </div>
                      </div>
                      <div className="bg-gray-50 rounded-lg p-4">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className="text-sm font-bold text-gray-900">
                            {Math.round(profile.dataConsistency || 0)}%
                          </span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div 
                            className="bg-green-600 h-2 rounded-full" 
                            style={{ width: `${profile.dataConsistency || 0}%` }}
                          ></div>
                        </div>
                      </div>
                      <div className="bg-gray-50 rounded-lg p-4">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-sm font-medium text-gray-700">Referential Integrity</span>
                          <span className="text-sm font-bold text-gray-900">
                            {Math.round(profile.referentialIntegrity || profile.relationshipIntegrity || 100)}%
                          </span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div 
                            className="bg-purple-600 h-2 rounded-full" 
                            style={{ width: `${profile.referentialIntegrity || profile.relationshipIntegrity || 100}%` }}
                          ></div>
                        </div>
                      </div>
                    </div>
                  ...
                  {/* Suspicious Records Alert */}
                  {profile.suspiciousRecords && profile.suspiciousRecords.length > 0 && (
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                      <div className="flex items-center space-x-2 mb-2">
                        <AlertTriangle className="h-5 w-5 text-yellow-600" />
                        <h5 className="text-sm font-medium text-yellow-800">
                          {profile.suspiciousRecords.length} Suspicious Records Detected
                        </h5>
                      </div>
                      <div className="text-sm text-yellow-700">
                        Records with potential data quality issues have been identified.
                        <button className="ml-2 text-yellow-800 underline">View Details</button>
                      </div>
                    </div>
                  )}
                  ...
                  {/* Column Profiles */}
                  <div>
                    <h5 className="text-lg font-medium text-gray-900 mb-4">Column Analysis</h5>
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                      {profile.columnProfiles.map((column: any) => (
                        <div key={column.columnId} className="border border-gray-200 rounded-lg p-4">
                          <div className="flex items-center justify-between mb-3">
                            <div>
                              <h6 className="font-medium text-gray-900">{column.columnName}</h6>
                              <div className="flex items-center space-x-2 text-sm text-gray-500">
                                <span>{column.dataType}</span>
                                {column.nullable && <span>• Nullable</span>}
                                {column.unique && <span>• Unique</span>}
                              </div>
                            </div>
                          ...
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              ...
                              <span className="font-medium">{(column.totalCount || 0).toLocaleString()}</span>
                            </div>
                            {column.nullCount !== undefined && (
                              <div className="flex justify-between">
                                ...
                                <span className="font-medium text-red-600">
                                  ...
                                  ({column.totalCount > 0 ? Math.round((column.nullCount / column.totalCount) * 100) : 0}%)
                                </span>
                              </div>
                            )}
                                ...
                                <span className="font-medium text-blue-600">
                                  ...
                                  {column.totalCount > 0 && (
                                    <span className="text-gray-500 ml-1">
                                      ({Math.round((column.uniqueCount / (column.totalCount - (column.nullCount || 0))) * 100) || 0}%)
                                    </span>
                                  )}
                                </span>
                              </div>
                            )}
                            {column.completeness !== undefined && (
                              <div className="flex justify-between">
                                <span className="text-gray-600">Completeness:</span>
                                <span className="font-medium text-green-600">{Math.round(column.completeness)}%</span>
                              </div>
                            )}
                            {column.validity !== undefined && (
                              <div className="flex justify-between">
                                <span className="text-gray-600">Validity:</span>
                                <span className="font-medium text-purple-600">{Math.round(column.validity)}%</span>
                              </div>
                            )}
                          </div>
                          ...
                          {column.qualityIssues && column.qualityIssues.length > 0 && (
                            <div className="mt-3 pt-3 border-t border-gray-200">
                              <div className="text-xs text-gray-600 mb-1">Quality Issues:</div>
                              <div className="flex flex-wrap gap-1">
                                {column.qualityIssues.slice(0, 3).map((issue: any, index: number) => (
                                  <span 
                                    key={index}
                                ...
                                {column.qualityIssues.length > 3 && (
                                  <span className="text-xs text-gray-500">+{column.qualityIssues.length - 3} more</span>
                                )}
                              </div>
                            </div>
                          )}
                          ...
                          {column.patterns && column.patterns.length > 0 && (
                            <div className="mt-3 pt-3 border-t border-gray-200">
                              <div className="text-xs text-gray-600 mb-1">Detected Patterns:</div>
                              <div className="text-xs text-gray-800">
                                {column.patterns[0].description} ({Math.round(column.patterns[0].percentage)}%)
                                {column.patterns.length > 1 && <span className="text-gray-500"> +{column.patterns.length - 1} more</span>}
                              </div>
                            </div>
                          )}
                        </div>
          ...
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="text-lg font-medium">No table profiles available</p>
            <p className="text-sm mt-2">Load a project with data to see detailed table profiles and data quality analysis.</p>
            {isGeneratingProfiles && (
              <div className="mt-4 text-blue-600">
                <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-blue-600 mx-auto mb-2"></div>
                <p className="text-sm">Analyzing data quality...</p>
              </div>
            )}
          </div>
        )}
